...
// mergeSetupFingerprints merges sources and generates from setup tasks
// into the parent task's Sources. This ensures that files managed by
// setup tasks (e.g. version enforcement) are included in the parent's
// fingerprint check. Both sources and generates go into Sources so that
// any change in setup-managed files triggers a parent rebuild.
