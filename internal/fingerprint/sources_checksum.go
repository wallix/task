...
// SourceValue returns the sources-only checksum for use as the CHECKSUM
// template variable. This is the same hash used by IsUpToDate to detect
// source changes and by the cache system to key cached artifacts.
func (checker *ChecksumChecker) SourceValue(t *ast.Task) (string, error) {
	sourcesGlobs, srcData := checker.filterChecksumData(t)
