...
func randomBytes(t *testing.T, n int, seed uint64) []byte {
	...
	data := make([]byte, n)
	_, _ = r.Read(data) // ChaCha8.Read never fails
	return data
}
...
func TestAssembleRejectsUnsafePath(t *testing.T) {
	...
	raw, _ := (&Index{
		...
		Chunker:     newIndex().Chunker,
		Compression: newIndex().Compression,
		Files:       []FileEntry{{Path: "/abs", Mode: 0o644}},
	}).Marshal()
...
func TestIndexRejectsForeignFormat(t *testing.T) {
	...
	corrupt := []struct {
		...
		{"setuid mode", func(idx *Index) {
			idx.Files = []FileEntry{{Path: "f", Mode: uint32(os.ModeSetuid) | 0o755}}
		}},
	}
