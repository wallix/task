...
// collectTaskGenerates returns all generated file paths for the given tasks
// and their dependency tree. Used by ImportCache to know which existing
// generates to clean up.
func (e *Executor) collectTaskGenerates(calls ...*Call) []string {
	checker := fingerprint.NewChecksumChecker(e.TempDir.Fingerprint)
	var generates []string
	seen := make(map[string]bool)

	var walk func(calls ...*Call)
	walk = func(calls ...*Call) {
		for _, call := range calls {
			t, err := e.CompiledTask(call)
			if err != nil {
				continue
			...
			for _, f := range st.GenerateFiles {
				if !seen[f] {
					seen[f] = true
					generates = append(generates, f)
				}
			}
			for _, dep := range t.Setup {
				if dep != nil {
					walk(&Call{Task: dep.Task, Vars: dep.Vars, Indirect: true})
				}
			}
			for _, dep := range t.Deps {
				if dep != nil {
			...
			for _, cmd := range t.Cmds {
				if cmd != nil && cmd.Task != "" {
					walk(&Call{Task: cmd.Task, Vars: cmd.Vars, Indirect: true})
				}
			}
		}
	}
