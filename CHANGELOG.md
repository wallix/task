...
- Fingerprint and cache checks now run **before** dependency tasks. Previously,
  deps (which may perform expensive builds) ran before checking whether the
  task was already up to date or available in the cache. This makes cached
  tasks with `sources: from: deps` skip dep execution entirely on cache hit.
